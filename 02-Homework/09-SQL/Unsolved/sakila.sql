...
#* 5a. You cannot locate the schema of the `address` table. Which query would you use to re-create it?  In this case created the script from the wizard
...
FROM
    ...
    FROM
        sakila.actor a, sakila.film b, sakila.film_actor c
    WHERE
...
FROM
   sakila.payment,sakila.customer where payment.customer_id = customer.customer_id
...
WHERE  film_id IN (SELECT film_id FROM  sakila.film_category
       WHERE
            category_id IN (SELECT category_id FROM sakila.category WHERE  name = 'Family'));  


# 7e. Display the most frequently rented movies in descending order.

SELECT 
    a.title, COUNT(*) AS 'rent_count'
FROM
    sakila.film a,
    sakila.inventory b,
    sakila.rental c
WHERE
    a.film_id = b.film_id
        AND c.inventory_id = b.inventory_id
GROUP BY b.film_id
...
DROP VIEW IF EXISTS sakila.vw_gross_revenue;

CREATE VIEW sakila.vw_gross_revenue  AS
SELECT 
    name, SUM(p.amount) AS gross_revenue
FROM
    sakila.category c
